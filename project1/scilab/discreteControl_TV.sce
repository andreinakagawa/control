...
Sdisc = [];
Kdisc = [];
S0 = diag([1200,0,0]); //Estimate for the Riccati matrix 
for k=1:length(t)-1
...
xlabel('Time (s)');
legend({'Angular position (10*x1)', 'Angular velocity (x2)', 'Current (x3)', 'Voltage (u)'},-1);
xs2jpg(gcf(), 'simulation_discrete_LTI_TV.jpg'); // Export to a JPG file
//------------------------------------------------------------------------------
